...
void bpf_jit_build_prologue(struct rv_jit_context *ctx)
{
	...
	if (gbpf_ready && ctx->prog->aux->gaux->gbpf_page) {
	  ...
	  //emit_sd(RV_REG_SP, GBPF_STK_SAVE_T4, RV_REG_S11, ctx);
	   ...
	   */
	  u64 bpf_stack = (u64) page_to_virt(ctx->prog->aux->gaux->gbpf_page);
	  bpf_stack &= 0x0000FFFFFFFFFFFF;
	  emit_imm(RV_REG_A0, bpf_stack, ctx);
	  emit_imm(RV_REG_S5, bpf_stack + PAGE_SIZE, ctx);
